...
DEFAULT_PORT=4567 # host port which container binds
DEFAULT_COMMAND="" # command which execute in container

# constant value
IMAGE_NAME=habitatra_docker
CONTAINER_NAME=habitatra
OPENHAB_CONTAINER_NAME=openhab4
...
function start(){
    ...
    -p 5678:4567 \
    -v $(pwd)/src:/var/www \
    --volumes-from $OPENHAB_CONTAINER_NAME \
    --name $CONTAINER_NAME \
    $IMAGE_NAME $COMMAND
}
